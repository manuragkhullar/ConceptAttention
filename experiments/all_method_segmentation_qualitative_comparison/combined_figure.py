...
if __name__ == "__main__":
    ...
    fig, axs = plt.subplots(2, len(model_names) + 1, figsize=(4 * (len(model_names) + 1), 4 * 2))
    ...
    axs[1, 0].imshow(target, cmap="viridis")
    axs[1, 0].axis("off")
    ...
    for model_name in model_names:
        ...
        axs[0, model_names.index(model_name) + 1].imshow(coefficients[0], cmap="viridis")
        axs[0, model_names.index(model_name) + 1].axis("off")
        axs[0, model_names.index(model_name) + 1].set_title(model_name)
        axs[1, model_names.index(model_name) + 1].imshow(mask[0], cmap="viridis")
        axs[1, model_names.index(model_name) + 1].axis("off")
        # Plot the coefficients
        # plt.figure()
    ...
    plt.savefig(f"results/{image_index}/all.png", bbox_inches="tight", pad_inches=0)